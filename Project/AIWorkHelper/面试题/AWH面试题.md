...
用传统方式的话每个场景都要写死接口逻辑，当需求碎片化时接口会爆发式增长，而且无法处理用户模糊的、组合式的需求。Agent 的核心价值是把'做什么'和'怎么做'解耦——用户说自然语言，Agent 自己规划要调哪些工具，这比我们硬编码所有场景要灵活得多
...
### 3. Agent 自动编排是怎么实现的？它是怎么决定调用哪些工具的？（核心亮点）
...
![](2026-05-10/1778408808_9cbc.png)


***

### 4. Agent 会不会陷入死循环呢？比如一直调用同一个工具？
...
### 6. 对话记忆是怎么管理的？如何实现不同用户的会话隔离？（核心亮点）
...
- 第一层是 会话路由层，用户发来请求，先由一个 LLM 判断这个请求要交给哪个 Handler 处理——是待办、是审批、还是知识库。这一层不关心上下文，只做意图分类。
- 第二层是 Agent 执行层，选定了 Handler 之后，Agent 会结合历史对话上下文，决定是否调用工具、调用哪个工具、怎么组织回答。
- 第三层是 记忆持久层，负责把每轮对话的输入输出存下来，供下一轮使用。这一层要解决的核心问题是：如何在 Token 限制下高效存储和检索多轮对话。"

##### 多用户隔离怎么做

- 项目里用用户 ID 作为会话的 key。每个用户第一次发起请求时，会给他创建一个独立的 Memory 实例，后续所有对话都关联到这个 ID。这样 A 用户和 B 用户的对话历史完全隔离，不会串。
- 在 Go 里实现的话，其实就是一个 map[userId]Memory 的结构，Go 的 sync.Mutex 保证并发安全。每个用户每次请求进来，从 map 里取他的 Memory 实例，取不到就新建一个。"

##### Token 爆炸问题怎么解决

对话轮次多了之后，把所有历史都塞进 Prompt 会导致 Token 爆炸，成本高、还超限。解决方案是 自动摘要压缩：

- 思路参考 LangChain 的 ConversationSummaryBufferMemory：维护两个区域——一个是原始消息列表，一个是摘要缓冲区。每次保存对话时，先统计一下当前 Token 总数有没有超过阈值。超过的话，调用一次 LLM 把历史对话压缩成一段摘要，摘要存到缓冲区，原始消息清空。这样 Prompt 里只需要带摘要 + 最近几轮消息，总量就控制住了。压缩过程对上层代码完全透明。"

##### 持久化怎么设计

纯内存存储有两个致命问题：服务重启数据全丢、无法跨设备同步。生产环境必须加持久层。主流方案是 MongoDB，结构大概是：

- 每个用户的每次对话作为一个 Document，包含会话 ID、用户 ID、时间戳、消息列表、摘要内容。下次用户请求进来，先从 MongoDB 把历史加载到内存，再参与后续对话。这样即使用户换设备，历史也在。
- 持久化时机也有讲究。最理想的是异步写入——对话结束后，把 Save 操作扔进一个队列，由后台 goroutine 批量写入 MongoDB，不阻塞主流程。读的时候先查 MongoDB，没有再走空初始化。"

##### 潜在问题和应对

###### 为什么要用用户 ID 作为 key，而不是会话 ID？

- 看业务场景。如果是单用户的跨会话多轮对话（比如连续多天），用会话 ID 更细粒度。但企业 OA 场景下，一个用户可能同时有多个会话，用用户 ID 作为 key 更简单，也够用。缺点是多会话会共享历史，如果需要严格隔离，加一层会话 ID 的逻辑就行。"

###### MongoDB 读写会成为瓶颈吗？

- 这是典型的读写放大问题。可以加 Redis 做二级缓存——用户请求进来先查 Redis，命中就直接返回，没命中再查 MongoDB 并回填 Redis。写入的时候异步写 MongoDB + 同步写 Redis 或者直接走消息队列。

###### 压缩摘要如果质量不好，导致上下文丢失怎么办？

- 这是 LLM 摘要的固有问题。可以在压缩前保留最近 N 轮不做压缩，只压缩更早的历史。另外也可以定期把摘要内容回写到原始消息列表做增量更新，而不是一直用同一段摘要

###### Memory 和 Router/Agent 是怎么串联的？

- 关键在 Agent 的 Prompt 模板。模板里预留一个 {{.history}} 变量，执行前从 Memory 里加载历史注入进去，Agent 执行后把本轮输入输出再 Save 回去。整个过程 Agent 完全感知不到有外部记忆注入，Prompt 里多了 history 字段，Agent 就自然看到了。
...
流式输出对用户体验非常有利，实际上需要在很多地方进行改造。首先 LLM 调用需要增加 API，OpenAI SDK 支持 Stream: true 参数，返回的是一个 channel，可以不断读取 token。在 Handler 层， 需要把 Agent 执行过程中的每个 token 通过 WebSocket 实时推送给客户端，这就要求用 WebSocket 而不是 HTTP。

具体流程是：客户端通过 WebSocket 发起请求，服务端启动 Agent 执行 → Agent 每生成一个 token 就通过 WebSocket 推送给客户端。技术上有几个关键点：一是请求限流，客户端的多个请求需要有一个 Agent 在后端服务池中并发执行；二是用户的每个请求都会通过 WebSocket 实时推送给客户端，流式的生命周期开启，如果客户端断开连接或者出现崩溃，要保证在 LLM 调用的流程中如果出错，需发送一个特定的错误通知给客户端。另外对流式输出的保存和恢复也有特殊要求，需要流式生成过程中可恢复。
...
知识库功能我们实现了基于 RAG的方案，分上传和检索两个环节：

- 上传时，用户上传文件后，Agent 会路由到知识库处理器，这个处理器会提取文本后做分块，我们用的是滑动窗口策略，chunkSize 500、chunkOverlap 50，意思是每500个字符切一块，相邻的块之间有50个字符的重叠，这样能避免关键信息被切断。分块完成后调用LLM的 Embedding 接口把每个块转成向量，然后存到 Redis 的向量索引里。我们用的是LangChain 的 redisvector，它底层是 Redis 的 RediSearch 模块，支持向量相似度搜索。
- 检索时用 了我们自定义的检索工具，它内部封装了 LangChain 的 RetrievalQA 链。用户提问会先转成向量，然后在 Redis 里做相似度搜索，检索出Top-1最相关的文本块（这个K值可以调），把文本块作为上下文喂给LLM，让它基于检索到的内容生成答案。
...
### 需要调用的工具很多或者调用链很长的话会不会出错或者出现延迟很高的场景，如果这样的话怎么解决

我们目前通过几个方向来优化：

- 超时熔断：给每个工具设置超时时间，超时直接跳过或降级
- 调用链路限制：限制 Agent 最多调用 N 次，防止无限循环
- 工具分类：把工具按耗时分组，耗时短的可以并发
  - 对于工具分类，可以考虑用 DAG（有向无环图）来描述工具依赖关系，实现真正的并行执行

    

### 有想过其他 agent编排方式方式吗，比如plan and execute，reflection和muti agent，为什么不用这些

我们选择了 ReAct 模式，因为企业内部办公助手的需求是单轮或少轮工具调用，任务边界清晰。Plan and Execute 虽然能处理复杂任务，但需要额外规划步骤，增加延迟和成本。Multi-Agent 适合更复杂的系统，当时MVP阶段不需要。ReAct 模式下，Agent 能自然地通过 Thought→Action→Observation 循环完成'意图识别→调用工具→获取结果'的流程，配合我们的 RouterChain 做路由，架构简洁且易于维护