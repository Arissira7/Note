...
首先 Agent 会观察当前状态，包括用户输入、已有的工具列表、历史对话记忆这些。我们在初始化时会给 Agent 注入一个 Prompt 模板（_defaultMrklPrefix），这个模板会自动列出所有可用工具的名称和描述，比如\
\
user_list 用于查询用户，time_parser 用于解析时间，todo_add 用于创建待办等工具。\
\
