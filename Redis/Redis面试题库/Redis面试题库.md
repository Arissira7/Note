> Redis 学习指引：
>
> - [ Redis 学习指引](https://ls8sck0zrg.feishu.cn/wiki/wikcnhADyULUGn6ZtleWm45rqvc)
> - [ Redis学习与面试](https://ls8sck0zrg.feishu.cn/wiki/wikcnefKuH9h5W6lDtiGi0E6gEX)
...
如果是讲区别可以从 <span style="color: inherit; background-color: rgba(255,246,122,0.8)">文件类型</span>，<span style="color: inherit; background-color: rgba(255,246,122,0.8)">文件恢复速度</span>，<span style="color: inherit; background-color: rgba(255,246,122,0.8)">安全性</span> 来进行回答，<span style="color: inherit; background-color: rgba(255,246,122,0.8)">但本质区别就是RDB是使用快照进行持久化</span>，AOF是日志。
...
[ 持久化介绍](https://ls8sck0zrg.feishu.cn/wiki/wikcnEvAUGue1C7LE9biLk0wBCg) （重点看）

[Redis persistence](https://redis.io/docs/management/persistence/)（官网，作为印证，辅助看）
...
所以，<span style="color: inherit; background-color: rgba(255,246,122,0.8)">查询布隆过滤器说数据存在，并不一定证明数据库中存在这个数据，但是查询到数据不存在，数据库中一定就不存在这个数据。</span>
